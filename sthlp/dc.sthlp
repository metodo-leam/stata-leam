...
{syntab:{it:International Classification of Diseases (ICD)}}
{synopt :{opth icd(string)}}use Stata ICD commands to check the values of the specified variables:{break}
{bf:icd(9)}: ICD-9-CM diagnostic codes{break}
{bf:icd(9p)}: ICD-9-CM procedure codes{break}
{bf:icd(10)}: ICD-10 diagnosis codes{break}
{bf:icd(10cm)}: ICD-10-CM diagnosis codes{break}
{bf:icd(10pcs)}: ICD-10-PCS procedure codes{p_end}
...
{p 4 4}
Execute {cmd: net install dc, from("https://raw.githubusercontent.com/metodo-leam/stata/master")} for install. 

{p 4 4}
This command uses the {help icd9}, {help icd9p}, {help icd10}, {help icd10cm} 
and {help icd10pcs} Stata commands.
...
net install dc, from("https://raw.githubusercontent.com/metodo-leam/stata/master"){p_end}
