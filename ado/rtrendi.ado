*! version 1.3.4  15dec2022 JM. Domenech, R. Sesma
/*
...
program define rtrendi, rclass
	...
	*/	 ORDinal(string) RC(string) NOne Wilson Exact WAld nst(string)]
	...
	*Ordinal
	if ("`ordinal'"=="") local ordinal = "expo"
	if ("`ordinal'"!="expo" & "`ordinal'"!="resp" ) print_error "ordinal() invalid -- invalid value" 
	local lk2 = ("`ordinal'"=="resp")
	if (`lk2' & `type'==3) print_error "ordinal(resp) option not compatible with case-control studies"
	if (`lk2' & "`data'"=="pt") print_error "ordinal(resp) option not compatible with person-time data"
	...
	*Get & check data
	...
	if (`nlist'<2) print_error "data invalid -- metric and a, n|b|t numlists must be present"
	...
	*Print header
	...
	if ("`data'"=="freq") {
		if ("`ordinal'"=="expo" & `type'!=2) di as res "(Risk data)"
		if ("`ordinal'"=="expo" & `type'==2) di as res "(Prevalence data)"
		if ("`ordinal'"=="resp") di as res "(Trend in proportions)"
	}
	...
	foreach i of numlist 1/`nstr' {
		...
		*Compute results
		if ("`data'"=="freq") rtrend__utils get_results_freq `type' `stat', d(`d') method(`ci_method') level(`level') rc(`rc') zero(`zero') k2(`lk2')
		if ("`data'"=="pt") {
			local printci = ("`exact'"!="")
		...
		*/	rc("`rc'") chi2P(`chi2Pearson') str(`i') nstr(`nstr') zero(`zero') k2(`lk2')
