*! version 1.1.8  15sep2025 JM. Domenech, R. Sesma
	...
	*Print table
	...
	if (`k'>2) {
		...
		*Print of kappa, observed agreement, standard errors and ci
		di as txt _newline _newline "{hline 12}{c TT}{hline 9}{c TT}{hline 8}{c TT}{hline 17}{c TT}{hline 11}{c TT}{hline 17}"
		di as txt "{col 13}{c |} Observed{c |}{col 32}{c |}  Standard Error {c |} One-sided {c |} " %4.0g `level' "% Asymptotic"
		di as txt "{col 6}Weight {c |}Agreement{c |}  Kappa {c |}{col 37}SE0 {c |}{col 46}SE1 {c |}  p value  {c |}  Conf. Interval"
		di as txt "{hline 12}{c +}{hline 9}{c +}{hline 8}{c +}{hline 8}{c +}{hline 8}{c +}{hline 11}{c +}{hline 17}" _c
		...
		*Get labels
		...
		foreach i of numlist 1/`len' {
			...
			foreach j of numlist 1/7 {
				...
				*local fmt = cond(`j'>5,"%7.4f","%6.4f")
				...
				if (`j'==1) {
					...
					if (`j'!= 5) di as res %7.4f `v' _c
					else di as res %8.4f `v' _c
				}
				if (`j'==1) di as txt " " _c
			...
			if (`i'==1 & "`ordered'"=="") {
				di as txt _newline "{hline 12}{c +}{hline 9}{c +}{hline 8}{c +}{hline 8}{c +}{hline 8}{c +}{hline 11}{c +}{hline 17}" _c
				di as txt _newline "{ralign 12:Categories:}{c |}{col 23}{c |}{col 32}{c |}{col 41}{c |}{col 50}{c |}{col 62}{c |}" _c
			}
			if (`r'[`i',6]<-1 | `r'[`i',7]>1) {
				di as txt "*" _c
				local warning 1
		...
		di as txt _newline "{hline 12}{c BT}{hline 9}{c BT}{hline 8}{c BT}{hline 8}{c BT}{hline 8}{c BT}{hline 11}{c BT}{hline 17}"
		...
		*unweighted kappa, standard error an ci
		di as txt _newline _newline "{hline 8}{c TT}{hline 17}{c TT}{hline 11}{c TT}{hline 20}"
		di as txt "{col 9}{c |}  Standard Error {c |} One-sided {c |}{col 42} " %4.0g `level' "% Asymptotic"
		di as txt "  kappa {c |}    SE0 {c |}    SE1 {c |}{col 30}p value  {c |} Confidence Interval"
		di as txt "{hline 8}{c +}{hline 8}{c +}{hline 8}{c +}{hline 11}{c +}{hline 20}"
		foreach j of numlist 2/7 {
			local fmt = cond(`j'>4,"%8.4f","%7.4f")
			*di as txt " " _c
		...
		di as txt _newline "{hline 8}{c BT}{hline 8}{c BT}{hline 8}{c BT}{hline 11}{c BT}{hline 20}"
...
real matrix get_kappa(string scalar type, real matrix d, real matrix k, real scalar level)
{
	...
	return((100*po,kappa,se0,se1,(1-normal(abs(z))),lb,ub))
}
end
