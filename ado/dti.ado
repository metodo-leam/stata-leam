*! version 1.2.2  15dec2022 JM. Domenech, R. Sesma
/*
...
program define dti, rclass
	...
	*Table data
	...
	if ("`st'"=="bt") {
		...
		local z
		if (`len'==4) local z = `prev'
		tempname pr
		local first 1
		foreach i in `z' `p' {
			getpv_bayes `i'/100 `se' `sp'

			* if available, print sample prevalence and results (v1.2.1)
			if (`first' & `len'==4) di as txt "Sample " as res %5.0g `i' as txt "%" _col(17) "{c |}" _c
			else di as res _col(7) %6.0g `i' as txt "%" _col(17) "{c |}" _c
			di as res _col(20) %7.0g r(pvp) as txt "%" _col(30) "{c |}" 	/*
