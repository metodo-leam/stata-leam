	...
	if ("`data'"=="freq") {
		...
		if (`type'<=2) {
			...
			if (!`k2') {
				...
				local c2 = cond("`rc'"=="f","No exposed","Exposed")
				if (`stat'==1) {
					di as txt " Resp {c |}" _col(17) "No" _col(27) "Exposed" _col(35) "NoExposed" _col(47) "Ref. category: `c2'"
	...
	*/				real scalar method, real scalar level, string scalar rc, string scalar zero, real scalar k2)
{
	...
	for (i=1; i<=len+1; i++) {
		...
		if (!k2) {
			...
			compute_ratio_k2(t[i],stat,rc,level)	//Ratio
		}
	}
...
mata:
//# compute_ratio_k2
void compute_ratio_k2(struct table2x2 t, real scalar stat, string scalar rc, real scalar level)
{
	if (stat==1) {
		//Risk Ratio
		if (rc=="f") t.ratio = (t.a1 / t.n) / (t.a0 / t.n0)
		if (rc=="l") t.ratio = (t.a0 / t.n0) / (t.a1 / t.n)
		t.se = sqrt(1/t.a1 - 1/t.n + 1/t.a0 - 1/t.n0)
	}
	if (stat==2) {
		//Odds Ratio
		if (rc=="f") t.ratio = (t.a1 / t.b1) / (t.a0 / t.b0)
		if (rc=="l") t.ratio = (t.a0 / t.b0) / (t.a1 / t.b1)
		t.se = sqrt(1/t.a0 + 1/t.b0 + 1/t.a1 + 1/t.b1)
	}
	t.ub = t.ratio * exp( invnormal((level+100)/200)*t.se)
	t.lb = t.ratio * exp(-invnormal((level+100)/200)*t.se)
