...
program define dc
	...
	*/	 icd(string) nomissing nodups varl(varlist) id(varlist)		/*
	...
	*/	("`ddu'"!="" & ("`ddl'"=="" | "`dd'"==""))) print_error "missing option dd, ddl or ddu"
	...
	if ("`icd'"!="" & "`icd'"!="9" & "`icd'"!="9p" & "`icd'"!="10" & /*
	*/	"`icd'"!="10cm" & "`icd'"!="10pcs") print_error "icd option with wrong value"
	if ("`icd'"!= "") {
		*Test if icd command exists
		capture which icd`cmd'
		if (_rc>0) {
			display in red "command icd`icd' not defined, ICD check is not available"
			exit 198
		}
	}
	if ("`icd'"!="" & ("`table'"!="" | "`vk'"!="")) print_error "too much options"
	...
	*Error variable value labels
	...
	if ("`isid'"!="") {
		...
		*There's errors
		...
		local i 1
		foreach var in `varlist'{
			tempvar vl_err`i' nd_err`i' dic_err`i' dd`i' dup_err`i' icd_err`i'
			...
			quietly {
				...
				*Get var type ("str" if var is string)
				...
				if ("`icd'"!= "") {
					*Check with icd Stata commands
					capture icd`icd' check `var', gen(`icd_err`i'')
					replace `err_v' = 2 if `icd_err`i''>0 & `icd_err`i''<.
				}
