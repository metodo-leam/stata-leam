...
program define rtrend, rclass
	...
	*/	[Data(string) ST(string) Zero(string) Level(numlist max=1 >50 <100) ORDinal(string) /*
	...
	*Ordinal
	...
	if ("`ordinal'"=="resp" & "`data'"=="pt") print_error "ordinal(resp) option not compatible with person-time data"
	...
	*Type of study & statistic
	...
	if ("`ordinal'"=="resp" & `type'==3) print_error "ordinal(resp) option not compatible with case-control studies"
	...
	*Reference category
	...
	if ("`ordinal'"=="resp" & ("`rc'"!="f" & "`rc'"!="l")) print_error "ordinal(resp) option is only compatible with rc(f) or rc(l)" 
	...
	*Get variable values
	...
	if ("`data'"=="freq") {
		...
		if ("`ordinal'"=="expo") {
			...
			if (`rk'!=2) print_error "response variable must have k=2 categories"
			if (`R'[1,1]!=0 | `R'[1,2]!=1) print_error "response variable must be binary (0,1)"
		...
		if ("`ordinal'"=="resp") {
			...
			if (`ek'!=2) print_error "exposure variable must have k=2 categories"
			if (`E'[1,1]!=0 | `E'[1,2]!=1) print_error "exposure variable must be binary (0,1)"
			if (`rk'<=2) print_error "response variable must have k>2 categories"
