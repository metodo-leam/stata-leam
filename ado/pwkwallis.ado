*! version 1.1.1 11jul2025 JM. Domenech, R. Sesma

/*
Kruskal-Wallis equality-of-populations rank test and non-parametric pairwise 
...
program define pwkwallis, rclass
	...
	*Kruskal-Wallis
	...
	preserve
	* Ri, ni with collapse
	quietly collapse (sum) Ri = `R' (count) ni = `R' if `touse', by(`by')		
	...
	* number of combinations of n elements chosing r: n!/(r!(n-r)!)
	* if r=2, ncomb = (n*(n-1))/2
	local ncomb = (`k'*(`k'-1))/2
	matrix `P' = J(`ncomb',4,.)
	local c 1
	local rowlbls ""
	...
	if (`adj') {
		* add column for manual bonferroni correction, NOT using _mtest command
		matrix `P' = `P' , J(`ncomb',1,.)
		forvalues i = 1/`ncomb' {
			matrix `P'[`i',5] = 1 - (1-`P'[`i',4])^(`k'*(`k'-1)/2)
		}
		*for Holm and Sidak correction, use _mtest command
		_mtest adjust `P', mtest(sidak) pindex(4) append
		_mtest adjust r(result), mtest(holm) pindex(4) append
		matrix `P' = r(result)
	}
	...
	* stored results
	...
	else matrix colnames `P' = Contrast t NonCorr Bonferr Sidak Holm
